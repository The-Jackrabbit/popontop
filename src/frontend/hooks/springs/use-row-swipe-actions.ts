...
export const useRowSwipeActions = ({
  ...
  const bind = useDrag(
    ({ active, movement: [offsetX] }) => {
      const isSwipeLengthOverThreshold =
        Math.abs(offsetX) > swipeLengthThreshold;
